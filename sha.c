...
void roundFunction(u_int64_t w, u_int64_t k) {
  u_int64_t a = H[0];
  u_int64_t b = H[1];
  u_int64_t c = H[2];
  u_int64_t d = H[3];
...
bool readBlock(FILE *fp, u_int64_t fileSize, u_int64_t offset) {
  for (int i = 0; i < 16; i++) {
    M[i] = 0;
  }
...
int main(int argc, char *argv[]) {
  ...
  u_int64_t offset = 0;
  while (readBlock(inputFP, inputSize, offset)) {
    offset += 128;
    compressionFunction();
  }
  compressionFunction();
